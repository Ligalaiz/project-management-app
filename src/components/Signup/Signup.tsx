...
const Signin: FC = () => {
  ...
  let content = 'Signup successful';
  if (checkUser) content = 'Пользователь с таким mail существует';
  if (isEdit) content = 'User edit successful';

  return (
    <div className="container" css={f.formWrap}>
      <form onSubmit={handleSubmit(onSubmit)} autoComplete="off" css={f.form} data-testid="form">
        <Field register={register} type="text" errors={errors} name="name" />
        <Field register={register} type="text" errors={errors} name="mail" />
        <Field register={register} type="text" errors={errors} name="password" />
        <Button disabled={isDisabled} type="submit" testid="formBtn" title={isEdit ? 'Edit' : 'Submit'} />
      </form>
